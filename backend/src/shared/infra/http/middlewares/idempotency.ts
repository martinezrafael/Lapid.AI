...
export async function ensureIdempotency(
  ...
  const cacheKey = `idempotency:notes:${idempotencyKey}`;
  ...
  const lockAcquired = await redisClient.set(
    ...
    JSON.stringify({
      status: 202,
      body: { message: "Processando rascunho em background..." },
    }),
    "EX",
  ...
  if (!lockAcquired) {
    return res.status(409).json({
      error:
        "Concorrência detectada. Esta requisição já está sob processamento ativo.",
    });
  }
